...
//Rare exceptions to indicate a bug in the _acstest code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_acstest = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_acstest]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.acstest.module"
